...
def display_html(size=3, content="content"):
  display(HTML(f'<h{size} style="text-align: center">{content}</h{size}>'))
...
def pair_plots(data, height=3, aspect=1.5, hue=None, legend=False):
  ...
  pair_grid = sns.PairGrid( data=data, aspect=aspect, height=height, hue=hue, corner=True)
...
def correlation_heatmap(data, figsize=(10, 5), method="spearman", cmap="RdBu"):
  ...
  hm = sns.heatmap( cm, vmin=-1, vmax=1, cmap=cmap, center=0, annot=True,fmt=".2f",
...
def num_summary(data, var):
  ...
  print(f"{'p-value':6}: {sw_pvalue}")
  print(f"{'Test Statistic':6}: {sw_statistic}")
  if sw_pvalue < alpha:
  ...
  print(f"{'Null Hypothesis':21}: The data is normally distributed")
  print(f"{'Alternate Hypothesis':21}: The data is not normally distributed")
  print(f"{'Critical Value':6}: {ad_critical}")
  print(f"{'Test Statistic':6}: {ad_statistic}")
...
def num_univar_plots(data, var, bins=10, figsize=(15, 6)):
  ...
  # cdf
  sns.ecdfplot(  data, x=var, ax=axes[1], color="green")
...
def plot_correlation_matrix(corr_matrix):
    ...
    mask[np.triu_indices_from(mask)] = True
    sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', vmin=0, vmax=1, fmt=".2f", square=True, linewidths=1.5, mask=mask )
